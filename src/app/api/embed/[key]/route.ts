...
export async function GET(
  ...
  try {
    ...
    const apiKey = resolvedParams.key.replace('.js', ''); // Remove .js extension
    ...
    const project = apiKeyData.projects;
...
// Simple hash function for API keys (in production, use crypto)
function hashApiKey(key: string): string {
  // This should match the hashing used when storing API keys
...
function generateWidgetScript(config: WidgetConfig): string {
  return `
(function() {
  'use strict';
  ...
  function createModal() {
    ...
    iframe.src = IFRAME_URL + '?theme=' + CONFIG.theme;
  ...
  window.addEventListener('message', function(event) {
    if (event.origin !== '${process.env.NEXT_PUBLIC_SITE_URL || 'https://signalsloop.com'}') {
      return;
    }
