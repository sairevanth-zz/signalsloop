...
export function CompetitiveAdminPanel({ projectId, onDataExtracted }: CompetitiveAdminPanelProps) {
  ...
  const handleExtractCompetitors = async () => {
    try {
      ...
      if (result.success) {
        toast.success(
          `✓ Extracted ${result.mentionsFound} competitor mentions from ${result.feedbackProcessed} feedback items`
        );
  ...
  const handleDetectFeatureGaps = async () => {
    try {
      ...
      const res = await fetch('/api/cron/detect-feature-gaps', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleGenerateRecommendations = async () => {
    try {
      ...
      const res = await fetch('/api/cron/strategic-recommendations', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ projectId }),
      });
      ...
      if (result.success) {
        toast.success(`✓ Generated ${result.recommendationsCreated} strategic recommendations`);
        if (onDataExtracted) onDataExtracted();
      } else {
  ...
  return (
    <Card className="border-2 border-dashed border-blue-200 bg-blue-50/50 p-6">
          ...
          <Card className="p-4 bg-white">
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-medium text-gray-900 text-sm mb-1">Extract Competitors</h4>
