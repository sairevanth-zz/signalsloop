  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Changelog Releases</h2>
          <p className="text-gray-600 dark:text-gray-400">Manage your product changelog releases</p>
        </div>
        <button
      ...
      {releases.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 dark:bg-slate-800/50 rounded-lg">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">No releases yet</h3>
        ...
        <div className="space-y-4">
          {releases.map((release) => (
            <div key={release.id} className="bg-white dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 rounded-lg p-6 shadow-sm">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{release.title}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`px-2 py-1 text-xs rounded-full ${release.release_type === 'major' ? 'bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-300' :
                        release.release_type === 'minor' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300' :
                          release.release_type === 'patch' ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' :
                            'bg-orange-100 dark:bg-orange-900/50 text-orange-800 dark:text-orange-300'
                      }`}>
                      {release.release_type}
                    </span>
                    {release.version && (
                      <span className="px-2 py-1 text-xs bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-gray-300 rounded-full">
                        v{release.version}
                      </span>
                    )}
                    <span className={`px-2 py-1 text-xs rounded-full ${release.is_published ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' : 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-300'
                      }`}>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
        <h3 className="text-blue-900 dark:text-blue-200 font-medium mb-2">Public Changelog URLs</h3>
        <div className="space-y-2 text-sm">
          <div>
            <span className="text-blue-800 dark:text-blue-300 font-medium">Main Changelog:</span>
            <span className="text-blue-600 dark:text-blue-400 ml-2">/{projectSlug}/changelog</span>
          </div>
          <div>
            <span className="text-blue-800 dark:text-blue-300 font-medium">RSS Feed:</span>
            <span className="text-blue-600 dark:text-blue-400 ml-2">/{projectSlug}/changelog/rss</span>
          </div>
        </div>
      </div>
    </div>
