...
export async function handleSpecAutoDrafted(event: DomainEvent): Promise<void> {
  ...
  console.log(`[NOTIFICATION AGENT] 📨 Spec auto-drafted: ${payload.title}`);
  ...
  try {
    ...
    if (!project) {
      console.log('[NOTIFICATION AGENT] ⏭️  Project not found');
      return;
    }
    ...
    if (!slackEnabled) {
      console.log('[NOTIFICATION AGENT] ⏭️  Slack notifications disabled for project');
      return;
    }
      ...
      blocks: [
        ...
        },
        {
          ...
          elements: [
            {
              type: 'mrkdwn',
              text: `🤖 Auto-generated by SignalsLoop AI • <${process.env.NEXT_PUBLIC_APP_URL || 'https://signalsloop.com'}${specUrl}|View Spec>`,
            },
          ],
        },
      ],
    ...
    console.log(`[NOTIFICATION AGENT] ✅ Notification sent in ${duration}ms`);
  } catch (error) {
    const duration = Date.now() - startTime;
    console.error(`[NOTIFICATION AGENT] ❌ Error after ${duration}ms:`, error);
  }
}
...
export async function handleThemeThresholdReached(event: DomainEvent): Promise<void> {
  ...
  console.log(`[NOTIFICATION AGENT] 📨 Theme threshold: ${payload.theme_name} (${payload.frequency} requests)`);
  ...
  try {
    ...
    if (!project || !project.settings?.notifications?.slack?.enabled) {
      console.log('[NOTIFICATION AGENT] ⏭️  Slack notifications disabled');
      return;
    }
    ...
    const message = {
      channel: project.settings?.notifications?.slack?.channel || '#product',
      text: `🔥 High-demand feature request: ${payload.theme_name}`,
      blocks: [
        {
          type: 'header',
          text: {
            type: 'plain_text',
            text: '🔥 High-Demand Feature Alert',
          },
        },
        {
          type: 'section',
        ...
        },
        {
          ...
          fields: [
            ...
            {
              type: 'mrkdwn',
              text: `*Urgency:*\n${urgencyLevel === 'high' ? '🔴 High' : urgencyLevel === 'medium' ? '🟡 Medium' : '🟢 Normal'}`,
            },
          ],
        },
        {
          ...
          elements: [
            {
              type: 'mrkdwn',
              text: `💡 A spec will be auto-drafted for this theme shortly`,
            },
          ],
        },
      ],
    ...
    const duration = Date.now() - startTime;
    console.log(`[NOTIFICATION AGENT] ✅ Theme alert sent in ${duration}ms`);
  } catch (error) {
    const duration = Date.now() - startTime;
    console.error(`[NOTIFICATION AGENT] ❌ Error after ${duration}ms:`, error);
  }
...
export async function handleHighVoteFeedback(event: DomainEvent): Promise<void> {
  ...
  const startTime = Date.now();
  console.log(`[NOTIFICATION AGENT] 📨 High-vote feedback: ${payload.vote_count} votes`);
  ...
  try {
    ...
    const message = {
      channel: project.settings?.notifications?.slack?.channel || '#product',
      text: `⬆️ Trending: ${post.title} (${payload.vote_count} votes)`,
      blocks: [
        {
          type: 'section',
          text: {
            type: 'mrkdwn',
            text: `⬆️ *Trending Feedback*\n\n*${post.title}*\n\n${payload.vote_count} users voted for this`,
          },
        },
        {
          type: 'actions',
    ...
    const duration = Date.now() - startTime;
    console.log(`[NOTIFICATION AGENT] ✅ High-vote alert sent in ${duration}ms`);
  } catch (error) {
    const duration = Date.now() - startTime;
...
async function sendSlackNotification(webhookUrl: string | undefined, message: any): Promise<void> {
  if (!webhookUrl) {
    console.log('[NOTIFICATION AGENT] ⏭️  No Slack webhook URL configured');
    return;
  }
  ...
  try {
    ...
    console.log('[NOTIFICATION AGENT] ✅ Slack notification sent');
  } catch (error) {
    console.error('[NOTIFICATION AGENT] ❌ Failed to send Slack notification:', error);
