...
/**
 * Send daily user intelligence digest email to admin
 * 
 * Includes:
 ...
 * - Notable signups
 */
export async function sendDailyIntelligenceDigest(params: {
  toEmail: string;
  stats: {
    totalSignups: number;
    planBreakdown: { free: number; pro: number; gift: number; discount: number };
  ...
  const content = `
    ...
    <tr>
      <td style="padding: 0 40px 30px;">
        ...
        <table role="presentation" style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="width: 50%; padding: 16px; background-color: #f0fdf4; border-radius: 8px; border: 1px solid #bbf7d0;">
              <div style="font-size: 28px; font-weight: 700; color: #166534; margin-bottom: 4px;">${stats.totalSignups}</div>
              <div style="font-size: 13px; color: #15803d; font-weight: 500;">New Signups</div>
            </td>
            <td style="width: 10px;"></td>
            <td style="width: 50%; padding: 16px; background-color: #eff6ff; border-radius: 8px; border: 1px solid #bfdbfe;">
              <div style="font-size: 28px; font-weight: 700; color: #1e40af; margin-bottom: 4px;">${Math.round(stats.enrichmentRate)}%</div>
              <div style="font-size: 13px; color: #1e40af; font-weight: 500;">Enrichment Rate</div>
            </td>
          </tr>
        </table>
      </td>
    ...
    <tr>
      <td style="padding: 0 40px 30px;">
        <h3 style="margin: 0 0 16px; font-size: 16px; font-weight: 600; color: #111827;">Plan Breakdown</h3>
        <table role="presentation" style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="padding: 12px; background-color: #f9fafb; border-radius: 6px;">
              <table role="presentation" style="width: 100%;">
                <tr>
                  <td style="font-size: 14px; color: #6b7280;">💎 Pro</td>
                  <td style="text-align: right; font-size: 16px; font-weight: 600; color: #111827;">${stats.planBreakdown.pro}</td>
                </tr>
                <tr><td style="height: 8px;"></td></tr>
                <tr>
                  <td style="font-size: 14px; color: #6b7280;">🎁 Gift</td>
                  <td style="text-align: right; font-size: 16px; font-weight: 600; color: #111827;">${stats.planBreakdown.gift}</td>
                </tr>
                <tr><td style="height: 8px;"></td></tr>
                <tr>
                  <td style="font-size: 14px; color: #6b7280;">🎟️ Discount</td>
                  <td style="text-align: right; font-size: 16px; font-weight: 600; color: #111827;">${stats.planBreakdown.discount}</td>
                </tr>
                <tr><td style="height: 8px;"></td></tr>
                <tr>
                  <td style="font-size: 14px; color: #6b7280;">🆓 Free</td>
                  <td style="text-align: right; font-size: 16px; font-weight: 600; color: #111827;">${stats.planBreakdown.free}</td>
                </tr>
              </table>
            </td>
          </tr>
      ...
      <td style="padding: 0 40px 30px;">
        ...
        <table role="presentation" style="width: 100%; border-collapse: collapse; background-color: #f9fafb; border-radius: 6px; padding: 12px;">
          ...
          <tr>
            ...
            <td style="padding: 8px 12px; text-align: right; font-size: 14px; color: #6b7280;">
              ${company.count} ${company.count === 1 ? 'signup' : 'signups'}
            </td>
          </tr>
          `
            )
      ...
      <td style="padding: 0 40px 30px;">
        ...
        <div style="margin-bottom: 16px; padding: 16px; background-color: #f0fdf4; border-radius: 8px; border-left: 4px solid ${getConfidenceColor(signup.confidence)};">
          ...
          <div style="margin-top: 8px;">
            ...
            ${signup.role ? `<div style="font-size: 13px; color: #374151;"><strong>Role:</strong> ${signup.role}</div>` : ''}
          </div>
          `
              : ''
          }
    ...
    <tr>
      <td style="padding: 0 40px 30px; text-align: center;">
        <a href="${APP_URL}/admin/user-intelligence" style="display: inline-block; padding: 14px 28px; background-color: #4f46e5; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px;">
          View Full Dashboard →
        </a>
      </td>
    </tr>
    ...
    <tr>
      <td style="padding: 0 40px 30px;">
        <p style="margin: 0; font-size: 13px; color: #6b7280; text-align: center;">
          ${stats.totalSignups === 0 ? 'No new signups in the last 24 hours.' : `Average confidence score: ${Math.round(stats.avgConfidence * 100)}%`}
        </p>
      </td>
    </tr>
  `;
  ...
  const html = buildEmailHtml({
    title: '🧠 Daily User Intelligence Report',
    greeting: `Here's your daily user intelligence digest for ${dateLabel}.`,
    paragraphs: [],
    outro: 'This report is automatically generated daily. Visit the admin dashboard for real-time data and detailed analytics.',
  }).replace(
    '<p style="margin:30px 0 18px; font-size:16px; line-height:1.6; color:#374151;">This report is automatically generated daily. Visit the admin dashboard for real-time data and detailed analytics.</p>',
    content
  );
  ...
  try {
    ...
    console.log('[Email] Daily intelligence digest sent successfully to:', toEmail);
    return { success: true, data };
  } catch (error) {
    console.error('Failed to send daily intelligence digest:', error);
    throw error;
  }
}
