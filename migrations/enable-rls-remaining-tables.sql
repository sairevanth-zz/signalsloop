...
-- ============================================================
-- 3. Slack Integration States Table
-- ============================================================
...
CREATE POLICY "Project owners can view slack_integration_states" ON slack_integration_states
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE si.id = slack_integration_states.integration_id
...
-- ============================================================
-- 5. Discord Integration States Table
-- ============================================================

ALTER TABLE discord_integration_states ENABLE ROW LEVEL SECURITY;

-- Service role has full access
CREATE POLICY "Service role can manage discord_integration_states" ON discord_integration_states
  FOR ALL USING (true) WITH CHECK (true);

-- Project owners can view their Discord integration states
CREATE POLICY "Project owners can view discord_integration_states" ON discord_integration_states
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM discord_integrations di
      JOIN projects p ON di.project_id = p.id
      WHERE di.id = discord_integration_states.integration_id
      AND p.owner_id = auth.uid()
    )
  );
...
BEGIN
  ...
  RAISE NOTICE '  - discord_integration_states';
  RAISE NOTICE '';
  RAISE NOTICE 'All tables now have RLS protection!';
  RAISE NOTICE '============================================================';
END $$;
